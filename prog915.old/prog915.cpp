...
void comp_move_hard( int &x, int &y, const TicTacToe board1, int player ) {
        ...
        for( j = 1 ; j <= 3 ; j++ ) {
            if( board2.checkSpace( i, j ) == 0 ) { //If there is no move there...
                if( player == 1 ) { //This is when the comp is player 1
                  ...
                  if( board2.checkWin() == 1 ) { //This means the move results in a win for the other player, if so take that move
                      x = i ;
                      y = j ;
                  }
                  else board2.removeMove( i, j ) ; //This means that the move wasnt a win
    ...
    err = board2.checkSpace( 2, 2 ) ; //Check to see if there is a move already there
    ...
    err = board2.checkSpace( 1, 1 );
    if( err == 0 ) {
        x = y = 1 ;
        return ;
    }
    err = board2.checkSpace( 1, 3 );
    ...
    //If all else fails have the easy ai take over and make a random move ( in one of the 4 spaces left)
    //comp_move_easy( y, x, board2 ) ;
...
int main() {
    ...
    int temp ;
    int x, y, player = 1, win = 0, comp = 0, user, difi = 0, comp_move = 0, fir ;
    char first ;
    TicTacToe board1 ;
    ...
    //Determine if the computer will play
    while( 1 ) {
    ...
    while( 1 ) {
        ...
        if( ( comp == 1 ) && ( fir == 1 ) && ( user == 1 ) ) comp_move = 1 ; // X is going first and the computer is player 1
        if( ( comp == 1 ) && ( fir == 2 ) && ( user == 2 ) ) comp_move = 1 ; // O is going first and the computer is player 1.

        //This happends if it is the human's move only
        if( ( comp_move == 0 ) || ( comp == 0 ) ) {
            while( 1 ) {
        ...
        //This is the easy ai
        if( ( comp == 1 ) && ( difi == 1 ) && ( comp_move == 1 ) ) comp_move_easy( x, y, board1 ) ;

        //This is the hard ai
        if( ( comp == 1 ) && ( difi == 2 ) && ( comp_move == 1 ) ) comp_move_hard( x, y, board1, player ) ;
